...
- `src/analysis/auto-retune.js` — sau 3 SL liên tiếp, lưu bằng chứng lúc vào lệnh rồi kiểm chứng candidate bằng chia lịch sử theo thời gian; chỉ tự ghi cấu hình khi PF dương, đủ mẫu và drawdown giảm.
  ...
  Nhánh này chạy ở production nhưng chỉ chẩn đoán (`autoApply: false`, `runtimeApply: false`): ngay lúc vừa chạm SL chưa đủ nến tương lai để phân biệt bị quét với sai hướng, nên không được tự nới SL. Quyền tự áp dụng nằm ở `daily-review` sau post-mortem. Các lớp bảo vệ vẫn giữ nguyên: candidate rủi ro dùng chung, bộ canh gác 4h và sàn cứng `minSlPercent` (3).
...
- `src/analysis/daily-review.js` — rà soát theo **thời gian**, so sánh mặc định 7 ngày và chỉ kích hoạt khi tỉ lệ SL vượt mục tiêu ở ít nhất 2 ngày có đủ mẫu. Chẩn đoán và chọn cặp dùng mẫu gộp liên ngày; từng candidate vẫn phải giảm SL trên cả train/holdout và giữ kỳ vọng dương ở bộ canh gác 4h. Alert runner gọi optimizer tối đa mỗi 24 giờ và ghi `activeTuning`; job Telegram dùng `--no-write` chỉ trình bày để tránh hai tiến trình cùng ghi state.

  Cửa sổ thống kê ngày do `dailyReview.windowMode` quyết định, **khác** `everyHours` (nhịp optimizer). Mặc định `calendar-day`, reset lúc 00:00 theo `dayOffsetHours` (VN = 7). Kèo **đang mở không bao giờ được tính**. Báo cáo vẫn giữ tổng quan riêng của ngày, còn cửa `minClosedTrades` và chẩn đoán dùng mẫu gộp `comparisonDays`; ngày không có kèo mới không được phép kích hoạt lại cùng một optimizer, và hai lần tự sửa cách nhau ít nhất `cooldownHours`.
