...
- `src/telegram/monitor.js` — vòng quét. Chỉ đánh giá lại **khi có nến mới đóng**; chỉ bắn khi có kèo thật (không bắn "đứng ngoài"/"chờ tín hiệu"). Có **cửa học sau SL** (`learning.pauseMinutesAfterStop`, mặc định 30 phút): vừa có kèo dính SL thì không mở kèo mới, kể cả các mã còn lại **trong chính lượt quét đó** — đây mới là chỗ nó cắn thật, vì nhịp cron ~3 tiếng. Cửa chỉ chặn việc MỞ kèo; theo dõi kèo đang chạy vẫn nguyên. Trạng thái nằm trong `auto-retune.json` vì đó là file duy nhất vừa được đồng bộ về `Trading-state` vừa được vòng quét ghi — thêm file mới sẽ không được runner khôi phục.
