...
Không có test suite và không có script `test`; chạy kiểm tra tay tương đương:
...
Khi chỉnh các file này, kiểm tra thủ công rằng chúng không import module chỉ có ở Node.
...
`models/` **được commit** để dashboard browser có model sẵn. `models/index.json` là manifest cho browser; chạy `npm run models:index` sau khi thêm hoặc xoá model.
